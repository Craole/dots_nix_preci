...
let
  ...
  inherit (userArgs.options)
    launcher
    terminal
    browser
    editor
    ;
  inherit (hostArgs) keyboard;
  modifier = toUpper keyboard.modifier;
in
{
  wayland.windowManager.hyprland.settings = {
    ...
    bindr = [
      #| Launcher
      "$MOD, $MOD_L, exec, pkill  ${launcher.primary.name} ||  ${launcher.primary.cmd}"
      "$MOD, SPACE, exec, pkill  ${launcher.secondary.name} || ${launcher.secondary.cmd}"
    ];
    ...
    bind =
      ...
      [
        ...
        #| Applications
        "$MOD,B,exec,pkill ${browser.primary} || ${browser.primary}"
        "$MODSHIFT,B,exec,${browser.secondary}"
        ...
        "$MOD,C,exec,${editor.gui}"
        "$MODSHIFT,C,exec,${editor.code}"
